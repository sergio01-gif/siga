...
<div class="max-w-xl mx-auto px-4 py-6">
    <h1 class="text-xl font-bold text-[#0072CE] mb-4">Editar Curso</h1>
    ...
    <form action="{{ route('cursos.update', $curso->id) }}" method="POST" class="space-y-4 bg-white p-6 rounded shadow">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="number" name="duracao" id="duracao" value="{{ old('duracao', $curso->duracao) }}"
                   class="mt-1 w-full px-3 py-2 border rounded">
        </div>
        ...
        <div>
            ...
            <textarea name="descricao" id="descricao" rows="3"
                      class="mt-1 w-full px-3 py-2 border rounded">{{ old('descricao', $curso->descricao) }}</textarea>
        </div>
        ...
        <div>
            ...
            <select name="status" id="status" class="mt-1 w-full px-3 py-2 border rounded">
                <option value="ativo" {{ old('status', $curso->status) === 'ativo' ? 'selected' : '' }}>Ativo</option>
        ...
        <div>
            ...
            <input type="number" name="coordenador_id" id="coordenador_id" value="{{ old('coordenador_id', $curso->coordenador_id) }}"
                   class="mt-1 w-full px-3 py-2 border rounded">
        </div>
